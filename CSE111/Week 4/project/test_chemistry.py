...
# Indexes for items in the periodic table dictionary values.
NAME_INDEX = 0
ATOMIC_MASS_INDEX = 1

# Indexes for items inside each pair in a symbol_quantity_list.
SYMBOL_INDEX = 0
QUANTITY_INDEX = 1


def test_make_periodic_table():
    """Verify that make_periodic_table returns a correct dictionary."""
...
def test_parse_formula():
    """Verify that parse_formula correctly converts chemical formulas."""
    periodic_table_dict = make_periodic_table()

    # Water: H2O -> [["H", 2], ["O", 1]]
    result = parse_formula("H2O", periodic_table_dict)
    result_dict = dict(result)
    assert result_dict["H"] == 2
    assert result_dict["O"] == 1

    # Glucose: C6H12O6 -> [["C", 6], ["H", 12], ["O", 6]]
    result = parse_formula("C6H12O6", periodic_table_dict)
    result_dict = dict(result)
    assert result_dict["C"] == 6
    assert result_dict["H"] == 12
    assert result_dict["O"] == 6

    # Salt: NaCl -> [["Na", 1], ["Cl", 1]]
    result = parse_formula("NaCl", periodic_table_dict)
    result_dict = dict(result)
    assert result_dict["Na"] == 1
    assert result_dict["Cl"] == 1

    # Benzene: C6H6
    result = parse_formula("C6H6", periodic_table_dict)
    result_dict = dict(result)
    assert result_dict["C"] == 6
    assert result_dict["H"] == 6

    # Iron oxide: Fe2O3
    result = parse_formula("Fe2O3", periodic_table_dict)
    result_dict = dict(result)
    assert result_dict["Fe"] == 2
    assert result_dict["O"] == 3


def test_compute_molar_mass():
    """Verify that compute_molar_mass returns correct molar masses."""
    periodic_table_dict = make_periodic_table()

    # Water H2O: 2*1.00794 + 1*15.9994 = 18.01528
    sym_qty = parse_formula("H2O", periodic_table_dict)
    assert compute_molar_mass(sym_qty, periodic_table_dict) == approx(18.01528, rel=1e-4)

    # Glucose C6H12O6: 6*12.0107 + 12*1.00794 + 6*15.9994 = 180.15588
    sym_qty = parse_formula("C6H12O6", periodic_table_dict)
    assert compute_molar_mass(sym_qty, periodic_table_dict) == approx(180.15588, rel=1e-4)

    # Benzene C6H6: 6*12.0107 + 6*1.00794 = 78.11184
    sym_qty = parse_formula("C6H6", periodic_table_dict)
    assert compute_molar_mass(sym_qty, periodic_table_dict) == approx(78.11184, rel=1e-4)

    # Salt NaCl: 22.98976928 + 35.453 = 58.44276928
    sym_qty = parse_formula("NaCl", periodic_table_dict)
    assert compute_molar_mass(sym_qty, periodic_table_dict) == approx(58.44276928, rel=1e-4)

    # Iron oxide Fe2O3: 2*55.845 + 3*15.9994 = 159.6882
    sym_qty = parse_formula("Fe2O3", periodic_table_dict)
    assert compute_molar_mass(sym_qty, periodic_table_dict) == approx(159.6882, rel=1e-4)
